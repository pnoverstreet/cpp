...
void CSocketServerDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST_RESULTS, m_cResults);
	//}}AFX_DATA_MAP
}
...
void CSocketServerDlg::OnListen() 
{
	if(!m_iListening)
	{
		m_oServer.Create(13524);
		m_oServer.Listen();
		m_cResults.AddString(_T("Listener started"));
	}
	else
	{
		m_oServer.Close();
		m_cResults.AddString(_T("Listener stopped"));
	}
}
